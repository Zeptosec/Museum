<template>
    <div>
        ...
        <h1 class="text-4xl text-center font-bold py-2">My museums</h1>
        <div v-if="museums.length > 0">
            <section class="grid gap-4 xl:grid-cols-2 pb-4">
                <MuseumCard v-for="museum in museums" :key="museum.id" @remove="deleteMuseum" :museum="museum"
                    :edit="userStore.user?.role === 'ADMIN'" :url="`/mymuseums/${museum.id}`" />
            </section>
        </div>
        ...
        <p v-else-if="museums.length === 0" class="text-center text-2xl text-error">No assigned museums</p>
...
async function fetchMuseums(pass = false) {
    ...
    pending.value = true;
    try {
        const rs = await AuthFetch(`${config.public.apiBase}/v1/museums/search?page=${pageParams.value.page}&pageSize=${pageParams.value.pageSize}`, {
            headers: {
                'authorization': `Bearer ${userStore.user?.accessToken}`
            }
        });
        if (rs.response.ok) {
            const data = rs.json as { museums: MuseumData[], hasNext: boolean };
            museums.value = data.museums;
            hasnext.value = data.hasNext;
        } else {
            error.value = getError(rs.json);
        }
    } catch (rr) {
        console.error(rr);
